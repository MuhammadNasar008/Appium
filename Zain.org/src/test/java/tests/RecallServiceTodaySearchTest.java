...
public class RecallServiceTodaySearchTest extends BaseTest {
	ArrayList<String> testStep = new ArrayList<String>();
	String TestName;
	String TestDescription;
	String TestCatagory;
	RecallsServiceTodaySearchPageScreen FindRecallsSearchPage = new RecallsServiceTodaySearchPageScreen();

	@Test(priority = 0)
	public void validateLoginFields() throws IOException, InterruptedException, ParseException {

		testStep.clear();
		TestName = "Validate Find Recalls Search Today Fields";
...
//Find Recalls page Validation
		...
		try {
			Waits.wait2Second();
			testStep.addAll(FindRecallsSearchPage.checkRecallSearchToday(Utility.getValue("findRecallsCarYear"),
